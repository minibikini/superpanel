  ...
  React.createClass
    ...
    getSections: ->
      ...
      sections = for rel in rels when rel.type is 'hasMany'
        ...
        <li><Link to={relRouteName} params={@getParams()}>{rel.getTitle()}</Link></li>

      activeCls = if @getRoutes()[3].isDefault then 'active' else ''
      <ul className="condense menu-bar primary">
        <li><Link to={schema.getRouteName 'Show'} activeClassName={activeCls} params={@getParams()}>Details</Link></li>
        {sections}
      </ul>
